...
!
! History
!  1986-92 Numerical Recipes Software "1"7%A2.
...
implicit none
  ...
  if( min(x,y)<0.0d0 .or. min(x+y,z)<TINY .or. max(x,y,z)>BIG) stop 'invalid arguments in rd'
  ...
  do iter = 1, max_iter
    ...
    dely=(ave-yt)/ave
    delz=(ave-zt)/ave
    if(max(abs(delx),dabs(dely),dabs(delz))<ERRTOL) exit
  enddo
  ea=delx*dely
  eb=delz*delz
  ec=ea-eb
  ed=ea-6.0d0*eb
  ee=ed+ec+ec
  rd=3.0d0*sum+fac*(1.0d0+ed*(-C1+C5*ed-C6*delz*ee)+delz*(C2*ee+delz*(-C3* ec+delz*C4*ea)))/(ave*dsqrt(ave))
