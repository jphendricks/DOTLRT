...
!
!...LOCAL VARIABLES...
    real(kind=dbl_kind) :: co2a_conc ! (mol m^-3) CAS CO2 concentration
    real(kind=dbl_kind) :: co2m_conc ! (mol m^-3) ref lev CO2 concentration
    real(kind=dbl_kind) :: rca       ! (-) C13/C12 ratio of canopy CO2 
    real(kind=dbl_kind) :: rcm       ! (-) C13/C12 ratio of ref level CO2
    real(kind=dbl_kind) :: c13cm     ! (TBD) concentration of C13 in mixed layer CO2
    real(kind=dbl_kind) :: c12cm     ! (TBD) concentration of C12 in mixed layer CO2 
    real(kind=dbl_kind) :: rcresp    ! (-) C13/C12 ratio of respiratory CO2
    real(kind=dbl_kind) :: c13resp   ! (mol m^-2 sec^-1) flux of C13 in respiratory CO2
    real(kind=dbl_kind) :: c12resp   ! (mol m^-2 sec^-1) flux of C12 in respiratory CO2
    real(kind=dbl_kind) :: c13ca     ! (TBD) concentration of C13 in canopy CO2 
    real(kind=dbl_kind) :: c12ca     ! (TBD) concentration of C12 in canopy CO2
...
! uses PV = nRT; at STP and 44.6 moles of gas per m3.  
    ...
    co2m_conc = sib%drvr%pco2m * (tref/sib%prog%ta)*(44.6/pref)
!
...
! notation and 13C/12C ratios.
    ...
    c13resp = (rcresp * sib%diag%resp_grnd) / (1. + rcresp)
    c12resp = sib%diag%resp_grnd / (1. + rcresp)
!
...
!==========================================================================
! this is the 'final' cfrax stuff to be done after the physiology 
! loops are finished.
...
!
! run though phys loop
    phys_loop: do i=1,5
      if(sib%param%physfrac(i) == 0.0) cycle phys_loop             
      sib%diag%c13assimn(6) = sib%diag%c13assimn(6) + sib%diag%c13assimn(i) * sib%param%physfrac(i)
      sib%diag%c12assimn(6) = sib%diag%c12assimn(6) + sib%diag%c12assimn(i) * sib%param%physfrac(i)
    enddo phys_loop

    sib%diag%rcassimn(6) = sib%diag%c13assimn(6)/sib%diag%c12assimn(6)
    sib%diag%d13cassimn(6) =  ((sib%diag%rcassimn(6)/pdb) - 1.0_dbl_kind)*1000.0_dbl_kind
!
! Canopy concentrations at time n+1 is calculated using an implicit scheme.
...
! and photosynthesis
!           sib%diag%flux13c = sib%diag%resp_grnd * rcresp / (1. + rcresp) -
