...
! local variables
  ...
  integer ichanidx, ncid
  real :: starthydro, finishhydro, t_time, t_tot_chan
  ...
  if (scantype=='hm_ph5') then
    ...
    do ichan=minchan, maxchan
      ...
      !do iph = 4,5
        ...
        !print*, 'atm(1)%clw  = ', atm(1)%clw
        !print*, 'atm(1)%rain = ', atm(1)%rain
        !print*, 'atm(1)%ice  = ', atm(1)%ice
        !print*, 'atm(1)%snow = ', atm(1)%snow
        !print*, 'atm(1)%grpl = ', atm(1)%grpl
        !print*, 'freq  = ', myfreq
        !print*, 'phase = ', phase
        !print*, 'tair  = ', tair
        ...
        !print*, "ix = ", ix, ", iy = ", iy
        myphase=iph
        tarray(ix) = value(yvar)
        parray(iy) = value(xvar)
