...
inp_theta = 55;       % observation angle from nadir (degrees)
num_streams = 8;      % total number of stream angles
num_sb_freqs = 1;     % number of frequency points per sideband
...
% DOTLRTv1.0    OUTPUT
% Tb(jud,jpol) = brightness temperature for observation level at observation angle
...
%     jud  = 1 (upwelling), 2 (downwelling)
%     jpol = 1 (horizontal), 2 (vertical)
% Tb_all[jz,jud,jpol] = brightness temperature profile at observation angle
...
[ Tb, dTb_dT, dTb_dp, dTb_dq, dTb_dw, Tb_all, tau ] = mex_mrt( inp_height, inp_theta,  num_streams, num_sb_freqs, ...
                                                      instr_spec, surface_reflectivity, nz, atm_inp);
tau