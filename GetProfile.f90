...
!  do ilev = 1, nlev
!    if(profile(ilev)/=0.d0) print*, ilev,profile(ilev), atm(ilev)%hgt
!  end do
...
! atm(ilev)%pressure    ! (mb) atmospheric pressure
! atm(ilev)%temperature ! (K) atmospheric temperature
! atm(ilev)%vapor_dens  ! (g m-3) water vapor mixing ratio
...
!
!====================================================================
subroutine construct_atmospheric_profile(ilon,ilat)
!====================================================================
...
! vertical atmospheric profile
  do ilev = 1, nlev
    atm(ilev)%hgt         =  height(ilon, ilat, ilev) ! (km)
    atm(ilev)%pressure    =  press(ilon, ilat, ilev)  ! (mb)
    atm(ilev)%temperature =  temp(ilon, ilat, ilev)   ! (K)
    atm(ilev)%vapor_dens  =  QVAPOR(ilon, ilat, ilev) ! (g/m^3)
...
!  nlev=nlev
  ...
  do ilev = 1, nlev
    ...
    atm(ilev)%pressure    = atminp(ilev,2) ! (mb)
    atm(ilev)%temperature = atminp(ilev,3) ! (K)
    atm(ilev)%vapor_dens  = atminp(ilev,4) ! (g/m^3)
...
! surface reflectance properties
  do iang = 1, nstream_surf
    surf_inp%theta(iang) = quad_angle(iang) ! (deg) stream angle
    surf_inp%vr(iang) = 0.05d0           ! (-) surface reflectivity in vert pol
    surf_inp%hr(iang) = 0.05d0           ! (-) surface reflectivity in hor pol
  enddo
