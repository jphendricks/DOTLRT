!=================SUBROUTINE RADA2======================================
subroutine rada2(sib,sib_loc,test,test10)
!=======================================================================
...
!   Respiration. Int. J. Remote Sensing, 6(8), 1335-1372.

    use kinds
    use sibtype
    use sib_const_module, only: zlnd
    ...
    real(kind=dbl_kind) :: a_nir  ! NIR reflectivity 
    real(kind=dbl_kind) :: a_vis  ! visible reflectivity
    real(kind=dbl_kind) :: difrat  !diffuse fraction
...
!...both satcap values are in meters: multiply by density to get kg/m^2...
...
!    sib%param%satcap(2) = 0.0002           ! lahouari
    ...
    if(sib%diag%areas < 0.25 .and. sib%stat%julday > 3 ) then
      sib%seas%snow_melt = MIN(sib%seas%snow_melt,sib%stat%julday)
    endif
...
a_nir=      sib%param%vcover*difrat          *(1.-sib%diag%radfac(1,2,2))
a_nir=a_nir+sib%param%vcover*(1.-difrat)     *(1.-sib%diag%radfac(1,2,1))
a_nir=a_nir+(1.-sib%param%vcover)*difrat     *(1.-sib%diag%radfac(2,2,2))
a_nir=a_nir+(1.-sib%param%vcover)*(1.-difrat)*(1.-sib%diag%radfac(2,2,1))

a_vis=      sib%param%vcover*difrat          *(1.-sib%diag%radfac(1,1,2))
a_vis=a_vis+sib%param%vcover*(1.-difrat)     *(1.-sib%diag%radfac(1,1,1))
a_vis=a_vis+(1.-sib%param%vcover)*difrat     *(1.-sib%diag%radfac(2,1,2))
a_vis=a_vis+(1.-sib%param%vcover)*(1.-difrat)*(1.-sib%diag%radfac(2,1,1))

a_nir=      sib%param%vcover*difrat          *(albedo(1,2,2))
a_nir=a_nir+sib%param%vcover*(1.-difrat)     *(albedo(1,2,1))
a_nir=a_nir+(1.-sib%param%vcover)*difrat     *(albedo(2,2,2))
a_nir=a_nir+(1.-sib%param%vcover)*(1.-difrat)*(albedo(2,2,1))

a_vis=      sib%param%vcover*difrat          *(albedo(1,1,2))
a_vis=a_vis+sib%param%vcover*(1.-difrat)     *(albedo(1,1,1))
a_vis=a_vis+(1.-sib%param%vcover)*difrat     *(albedo(2,1,2))
a_vis=a_vis+(1.-sib%param%vcover)*(1.-difrat)*(albedo(2,1,1))

test10(1)=a_nir
test10(2)=a_vis
test10(3)=(a_nir-a_vis)/(a_nir+a_vis)
test10(4)=sib%diag%radfac(2,1,2)
test=test10(1)
    !
    !----------------------------------------------------------------------
    !
