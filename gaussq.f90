...
!        29halog    x is zero or negative, 29, 1, 2)
!
      call r9upak (x, y, n)
!
      xn = n - 1
...
!
 10   continue ! if (x.lt.10.d0) call seteru (23hd9lgmc  x must be ge 10, 23, 1, 2)
...
!        31hd9pak   packed number overflows, 31, 1, 2)
!
      if (nsum.eq.0) return
...
!
 30   d9pak = 2.0d0*d9pak
      ...
      return
!
 40   continue ! call seteru (32hd9pak   packed number underflows, 32, 1, 0)
      d9pak = 0.0d0
      return
!
      end
...
!        25hdcsevl  x outside (-1,+1), 25, 1, 1)
!
      twox = 2.0d0*x
      b1 = 0.d0
      b0 = 0.d0
...
!        31hdexp    x so big dexp overflows, 31, 2, 2)
!
      xint = dint (x)
      y = x - xint
!
...
!
      ...
      return
!
 20   continue ! call seteru (34hdexp    x so small dexp underflows, 34, 1, 0)
      dexp = 0.d0
      return
!
      end
...
!        31hdgamma  x is a negative integer, 31, 4, 2)
!      if (x.lt.(-0.5d0) .and. dabs((x-dint(x-0.5d0))/x).lt.dxrel) call &
!        seteru (68hdgamma  answer lt half precision because x too near &
!      negative integer, 68, 1, 1)
!      if (y.lt.xsml) call seteru ( &
!        54hdgamma  x is so close to 0.0 that the result overflows, &
!        54, 5, 2)
!
      do 20 i=1,n
...
!
 50   continue !if (x.gt.xmax) call seteru (32hdgamma  x so big gamma overflows, &
!        32, 3, 2)
!
      dgamma = 0.d0
!      if (x.lt.xmin) call seteru (35hdgamma  x so small gamma &
!        underflows  , 35, 2, 0)
      if (x.lt.xmin) return
!
...
!        61hdgamma  answer lt half precision, x too near negative &
!        integer , 61, 1, 1)
!
      sinpiy = dsin (pi*y)
...
!
      ...
      return
!
 50   continue !call seteru (68hdint    dabs(x) may be too big to be represented &
!        as an exact integer, 68, 1, 1)
      dint = x
      return
!
      end
...
!        29hdlog    x is zero or negative, 29, 1, 2)
!
      call d9upak (x, y, n)
!
      xn = n - 1
...
!        42hdsin    no precision because abs(x) is big, 42, 2, 2)
!      if (y.gt.xwarn) call seteru ( &
!        54hdsin    answer lt half precision because abs(x) is big, &
!        54, 1, 1)
!
      dsin = x
      if (y.lt.xsml) return
...
!
      ...
      return
!
 20   continue ! if (x.lt.0.d0) call seteru (21hdsqrt   x is negative, 21, 1, 1)
      dsqrt = 0.0d0
      return
!
      end
...
!        35hinitds  number of coefficients lt 1, 35, 2, 2)
!
      err = 0.
      do 10 ii=1,nos
        i = nos + 1 - ii
 ...
 10   continue
!
 20   continue !if (i.eq.nos) call seteru (28hinitds  eta may be too small, 28, &
!        1, 2)
      initds = i
!
      return
...
!
!      if (nos.lt.1) call seteru ( &
!        35hinits   number of coefficients lt 1, 35, 2, 2)
!
      err = 0.
      do 10 ii=1,nos
        i = nos + 1 - ii
 ...
 10   continue
!
 20   continue ! if (i.eq.nos) call seteru (28hinits   eta may be too small, 28, &
!        1, 2)
      inits = i
!
      return
